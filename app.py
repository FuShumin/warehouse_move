...
def fetch_and_calculate(client_id):
    ...
    try:
        ...
        # 执行SQL查询
        (
            df_special_rules, df_current_stock, df_max_stock, df_onload_stock, df_min_safe, df_max_safe,
            df_item_attributes,
        ...
        # 找出所有仓库和对应的最大库存
        max_stock_per_warehouse = df_max_stock[df_max_stock['warehouse_code'].isin(unique_warehouses)][
            'safe_stock'].values
        small_value = 1e-9
        ...
        # 生产计划调整预入库
        expected_incoming = summarize_production_plans(df_produce, df_produce_mapping, day_range=2)
        ...
        max_safety_stock = update_safety_stock(expected_incoming, warehouse_to_index, item_to_index, max_safety_stock,
                                               min_safety_stock)
        # 数据校验
        check_for_errors(current_stock, max_stock_per_warehouse, min_safety_stock, max_safety_stock, df_warehouse_info,
        ...
        for start_warehouse_idx, target_warehouse_idx, item_code_idx, transfer_qty in actions:
            ...
            row_dict = {
                ...
                "cigarette_type": int(item_attribute_id) if isinstance(item_attribute_id,
                                                                       np.int64) else item_attribute_id,
        ...
        # 数据校验错误处理
        return "data_validation_error", str(e)
...
def get_final_report():
    ...
    try:
        result = fetch_and_calculate(client_id)

        if isinstance(result, tuple):  # 检查是否是错误响应
            error_type, error_message = result
            if error_type == "database_error":
                return jsonify({"code": -1, "message": f"数据库查询错误: {error_message}"}), 400
            elif error_type == "data_validation_error":
                return jsonify({"code": -1, "message": f"数据校验错误: {error_message}"}), 400
            elif error_type == "optimization_error":
                return jsonify({"code": -1, "message": f"规划过程错误: {error_message}"}), 400
