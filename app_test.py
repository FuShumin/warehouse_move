...
from linear_programming_test import *
...
# %%
    ...
    with get_database_connection() as conn:
        with conn.cursor() as cursor:
            ...
            query = """SELECT swi.code, pwp.priority
                        FROM stock_warehouse_info AS swi
                        JOIN lcs_dispatch_cp_warehouse_priority AS pwp ON swi.code = pwp.warehouse_code
                        WHERE swi.client_id = %s AND swi.is_delete=0 AND swi.safe_stock IS NOT NULL;
                        """
            df_priorities = execute_query(cursor, query, (client_id,))  # 优先级
    ...
    # SECTION 优先级处理
    df_priorities.rename(columns={'code': 'warehouse_code'}, inplace=True)
    df_priorities['priority'] = pd.to_numeric(df_priorities['priority'], errors='coerce')
    df_priorities['index'] = df_priorities['warehouse_code'].map(warehouse_to_index)
    priorities = df_priorities.sort_values('index')
